...
class MtnMomoService
{
    ...
    {
        $this->config = AppSetting::get('payment_provider_mtn_money', []);
    }
    ...
    public function getToken(): string
    {
        ...
        if (!$apiUser || !$apiKey || !$subKey) {
            throw new RuntimeException("MTN MoMo n'est pas configuré.");
        }

        return Cache::remember('mtn_momo_token_' . md5($apiUser), 3300, function () use ($apiUser, $apiKey, $subKey) {
    ...
    public function requestToPay(float $amount, string $planCurrency, string $phone, string $externalId, string $payerMessage = 'Abonnement GazManager'): string
    {
        ...
        $response = Http::withToken($token)
            ...
            ->post($this->baseUrl() . '/collection/v1_0/requesttopay', [
                ...
                'currency'     => $this->currency($planCurrency),
        ...
        if ($response->status() !== 202) {
            throw new RuntimeException('MTN MoMo a refusé la demande de paiement.');
        }
    ...
    public function checkStatus(string $referenceId): array
    {
        ...
        if (!$response->successful()) {
            throw new RuntimeException('Impossible de vérifier le statut du paiement MTN.');
        }
